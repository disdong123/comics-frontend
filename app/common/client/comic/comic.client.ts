...
import { ComicType } from '@/app/common/client/comic/type';

type ComicRatingType = {
  comicId: string;
  rating: number;
};

export class ComicClient {
  static async getComicById(id: string): Promise<ComicType> {
    return (await this.getComics()).find((comic) => comic.id === id);
  }

  static async getRatingsByIds(comicIds: string[]): Promise<ComicRatingType[]> {
    return comicIds.map((comicId, i) => ({
      comicId,
      rating: Number((Math.random() * 6).toFixed(2)),
    }));
  }
